...
class BertForDeprelSentenceOutput:
    """Each prediction tensor has size (T, C), where T is the maximum sequence length
    for the containing batch, and C is the number of classes being assigned a
    probability for the particular tensor.
    """
...
class BertForDeprelBatchOutput:
    """Each prediction tensor has size (B, T, C), where B is the number of sentences in
    the batch."""
    ...
    def detach(self):
        """Return a new result with all of the Tensors detached from backprop (used for
        prediction)."""
    ...
    def distributions_for_sentence(
        self, sentence_idx: int
    ) -> BertForDeprelSentenceOutput:
