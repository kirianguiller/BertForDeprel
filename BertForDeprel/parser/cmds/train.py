...
class Train(CMD):
    ...
    def add_subparser(self, name, parser):
        ...
        subparser.add_argument('--batch_size', default=16, type=int,
                                help='batch_size to use')
        subparser.add_argument('--punct', action='store_true',
                               help='wether to include punctuation')        
        subparser.add_argument('--ftrain', required=True,
                               help='path to train file')
        subparser.add_argument('--fdev', default='',
                               help='path to dev file')
        subparser.add_argument('--ftest', default='',
                               help='path to test file')
        subparser.add_argument('--freeze_bert', action='store_true',
                               help='path to test file')
        subparser.add_argument('--fpretrain', default='',
                               help='path to pretrain model')
        subparser.add_argument('--reinit_bert', action='store_true',
                        help='path to test file')
        subparser.add_argument('--compute_fields', action='store_true',
                        help='compute new fields from conllu or not')
        subparser.add_argument('--keep_epoch', action='store_true',
                        help='compute new fields from conllu or not')
        subparser.add_argument('--split_deprel', action='store_true',
                               help='wether to split relations for predicting')
        subparser.add_argument('--n_to_train', default=0, type=int,
                                help='number of sequences to use for parsing (used in the experience 1/10/100/1000...')
        subparser.add_argument('--n_to_test', default=0, type=int,
                                help='number of sequences to use for parsing (used in the experience 1/10/100/1000...')
    ...
    def __call__(self, args):
        ...
        if args.fpretrain:
            ...
            list_conllus = []
            if args.compute_fields:
                path_train = os.path.join(args.folder, "train")
                for file in os.listdir(path_train):
                    list_conllus.append(os.path.join(path_train, file))
            else:
                list_conllus.append(args.ftrain)
            
            
            list_deprel_main, list_deprel_aux = create_deprel_lists(*list_conllus, split_deprel=args.split_deprel)
            list_pos = create_pos_list(*list_conllus)
        ...
        print(f"{'train:':6} {len(train_dataset):5} sentences, "
              f"{len(train_loader):3} batches, ")
        print(f"{'test:':6} {len(test_dataset):5} sentences, "
        ...
        n_epoch_start = 0
        if (args.fpretrain != '') & (args.keep_epoch == True):
        ...
        for n_epoch in range(n_epoch_start+1, args.epochs+1):
          ...
          if results["LAS_epoch"] > LAS_best:
            ...
            if epochs_no_improve >= args.patience:
              print("Earlystopping ({} epochs without improvement)".format(args.patience))
              break
