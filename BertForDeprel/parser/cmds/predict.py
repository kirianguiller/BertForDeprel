...
from parser.utils.train_utils import eval_epoch, deprel_aligner_with_head, eisner
from parser.utils.os_utils import path_or_name
from parser.utils.chuliu_edmonds_utils import chuliu_edmonds_one_root
...
class Predict(CMD):
    ...
    def add_subparser(self, name, parser):
        ...
        subparser.add_argument('--fpred', default='',
                               help='path to dataset')
        subparser.add_argument('--upostag', action='store_true',
                               help='whether to predict POS')
        subparser.add_argument('--multiple', action='store_true',
                               help='whether to include punctuation')
        subparser.add_argument('--write_preds_in_misc', action='store_true',
                               help='whether to include punctuation')
    ...
    def __call__(self, args):
        ...
        for path in paths_pred:
            ...
            print(f"{'eval:':6} {len(pred_dataset):5} sentences, "
              f"{len(pred_loader):3} batches, ")
            start = timer()
            list_conllu_sequences = []
            with torch.no_grad():
                for n_sentence, (seq, subwords_start, attn_masks, idx_convertor) in enumerate(pred_dataset):
                    ...
                    deprels_main_pred_chuliu = deprel_aligner_with_head(deprels_main_pred, chuliu_heads_pred)
                    deprels_main_pred_chuliu_list = deprels_main_pred_chuliu.max(dim=1)[1][subwords_start==1].tolist()

                    deprels_aux_pred_chuliu = deprel_aligner_with_head(deprels_aux_pred, chuliu_heads_pred)
                    deprels_aux_pred_chuliu_list = deprels_aux_pred_chuliu.max(dim=1)[1][subwords_start==1].tolist()
                    ...
                    for n_token in range(len(conllu_sequence)):
                        ...
                        if type(conllu_sequence[n_token - poped_item]['id']) != int:
                            ...
                            poped_item += 1
                    for n_token, (pos_index, head_chuliu, dmpmstn, dapmst) in enumerate(zip(poss_pred_list,chuliu_heads_list, deprels_main_pred_chuliu_list, deprels_aux_pred_chuliu_list)):
                        ...
                        if args.write_preds_in_misc:
                            ...
                            if loaded_args.split_deprel:
                                ...
                                if args.i2dra[dapmst] == 'none':
                                    ...
                                    token["deprel"] = "{}:{}".format(args.i2drm[dmpmstn], args.i2dra[dapmst]) 
                            else :
                                token["deprel"] = args.i2drm[dmpmstn]
            ...
            with open(path_result_file, 'w') as f:
                f.writelines([sequence.serialize() for sequence in list_conllu_sequences])
